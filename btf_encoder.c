...
static int32_t structure_type__encode(struct btf_elf *btfe, struct cu *cu, struct tag *tag, uint32_t type_id_off)
{
	...
	name = active_loader->strings__ptr(cu, type->namespace.name);
	...
	type__for_each_data_member(type, pos) {
		 ...
		 * is required.
		 */
		name = active_loader->strings__ptr(cu, pos->name);
...
static int32_t enumeration_type__encode(struct btf_elf *btfe, struct cu *cu, struct tag *tag)
{
	...
	name = active_loader->strings__ptr(cu, etype->namespace.name);
			   ...
			   uint32_t array_index_id, uint32_t type_id_off)
{
	...
	case DW_TAG_base_type:
		name = active_loader->strings__ptr(cu, tag__base_type(tag)->name);
	...
	case DW_TAG_typedef:
		name = active_loader->strings__ptr(cu, tag__namespace(tag)->name);
		   ...
		   bool skip_encoding_vars)
{
	...
	cu__for_each_function(cu, core_id, fn) {
		...
		name = active_loader->strings__ptr(cu, fn->name);
